...
/*   Updated: 2016/01/18 15:48:37 by hbeaujou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_line_5(t_function **file, t_tempo *vars, int test)
{
	...
	ft_lstaddend_instruct(&tmp->lines->content, tmp3);
	tmp3 = new_instruct(vars->param5, check_params(vars->param5), 0);
	ft_lstaddend_instruct(&tmp->lines->content, tmp3);
}
...
void	make_line_6(t_function **file, t_tempo *vars, int test)
{
	...
	tmp3 = new_instruct(vars->param5, check_params(vars->param5), 0);
	ft_lstaddend_instruct(&tmp->lines->content, tmp3);
	tmp3 = new_instruct(vars->param6, check_params(vars->param6), 0);
	ft_lstaddend_instruct(&tmp->lines->content, tmp3);
}
